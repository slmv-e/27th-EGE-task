...
with open("27-B.txt") as f:
    ...
    for _ in range(n):
        ...
        # сначала считываем строку и с помощью функции "split" разбиваем строку на массив с двумя элементами
        # этот массив мы передаем в функцию map, которая преобразует тип каждого элемента массива из "string" в "int"
        # далее мы преобразуем массив в кортеж, так как мы не будем менять содержимое пары
...
# начнем с lambda выражения, оно позволяет создавать анонимные функции, которые не привязаны по к имени.
# в функцию "sort" мы передаем в качестве аргумента key анонимную функцию, которая возвращает кортеж с двумя аргументами
# и по этим двум аргументам функция "sort" сортирует массив
